...
def main():
    ...
    parser.add_argument("--print_model_config", action="store_true", help="Print the config of the text model.")
    ...
    data_file = text_features_path / f"{full_ident}.h5"
    ...
    for key, value in dict(dataset[0]).items():
        print(f"{key}: {value}\n")
    ...
    temp_file = text_features_path / f"TEMP_{utils.get_timestamp_for_filename()}.h5"
    data_h5 = h5py.File(temp_file, "w")
    lengths = {}
    total_feat_dim = None
    for i, batch in tqdm(enumerate(dataloader), desc="compute text features", total=maths.ceil(
            len(dataset) / args.batch_size)):  # type: TextDataBatchPoint
        if args.cuda:
        ...
        # all hidden states are a list of tensors with shape (batch_size, max_sent_len, feat_dim) per tensor
        ...
        for batch_num in range(batch_size):
            ...
            assert feature.shape[0] == sum(sentence_lengths), (
                f"Feature sequence length {feature.shape[0]} is not equal sum of the sentence lengths: "
                f"{sentence_lengths}")
...
class TextConverterDataset(data.Dataset):
    ...
    def __getitem__(self, item: int) -> TextDataPointTuple:
        ...
        # check max length is fulfilled
        if sum(len(sentence_tokens) for sentence_tokens in par_tokens) > self.max_text_len:
