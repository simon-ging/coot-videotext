...
class BertTextFeatureLoader:
    def __init__(
            self, dataset_path: Path, ids: List[str],
            metadata_name: str = "default", preload=True):
...
class Youcook2VideoFeatureLoader:
    def __init__(
            self, dataset_path: Path, dataset_features: str, ids: List[str],
            preload: bool):
        self.dataset_path = Path(dataset_path)
...
class VideoDatasetFeatures(data.Dataset):
    def __init__(
            self, dataset_name: str, dataset_path: Path, dataset_features: str,
        ...
        meta_file = dataset_path / f"meta_{dataset_features}.json"
        ...
        if dataset_name == "activitynet":
            ...
            self.vid_data = Youcook2VideoFeatureLoader(
                dataset_path, dataset_features, self.ids, preload_vid_feat)
        else:
